...
name: api-generator
description: >
  USE WHEN generating REST API endpoints, creating OpenAPI specifications,
  or scaffolding backend routes from specifications or requirements.
version: "1.0.0"
...
compatibility:
  claude-code:
    status: full
    notes: "Full support with subagent isolation for parallel endpoint generation"
  windsurf:
    status: partial
    notes: "No subagent isolation; sequential generation only"
  github-copilot:
    status: partial
    notes: "10-file working set limit; may need to batch large APIs"

emulation:
  subagents:
    pattern: sequential-workflow
    fallback: "Generate endpoints sequentially with progress markers"
    limitations:
      - "No parallel generation"
      - "Longer execution time for large APIs"
  hooks:
    pattern: manual-verification
    fallback: "Include manual lint/validate steps in workflow"
    limitations:
      - "No automatic post-write linting"
      - "User must run validation manually"
---
